...
/************************************************************************/
DWORD WINAPI GetAllPortByProcessId(TcpOrUdp type, DWORD dwProcessId, DWORD * dwAllPort, DWORD dwMaxLen)
{
	DWORD dwPortCount = 0;
	HMODULE hModule = LoadLibraryW(L"iphlpapi.dll");
	if (hModule == NULL)
...
DWORD WINAPI GetAllPortByProcessByName(TcpOrUdp type, const char* pszProcessName, DWORD * dwAllPort, DWORD dwMaxLen)
{
	DWORD id = 0; 
	...
	if (id > 0)
	{
		return GetAllPortByProcessId(type, id, dwAllPort, dwMaxLen);
	}
	return 0;
}
